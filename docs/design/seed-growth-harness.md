...
|---|---|---|
| **G1 — Growth harness** ([#203](https://github.com/philipdallen/rubato/issues/203)) | `tools/muse_grow/`: L1→L4→delta→compare pipeline + growth report JSON | L1 generate loop (real); stand-in marked until then |
| **G2 — Workbench growth view** ([#204](https://github.com/philipdallen/rubato/issues/204)) | growth report rendered per seed (trait trajectories across iterations) | G1 |
| **G3 — Iteration fixtures** ([#205](https://github.com/philipdallen/rubato/issues/205)) | two committed seed revisions + their deltas so G1's compare has known-answer tests | G1 |
| **G4 — Expansion-time logging** ([#252](https://github.com/philipdallen/rubato/issues/252)) | log `expansion_time_ms` per `operation` tag against `(variation_point_count, note_count)` per `grow_one` call — measurement toward a future expansion-cost estimate (proposal: [lineage chain](proposal-lineage-chain.md) §2.4) | S3.7 ([#248](https://github.com/philipdallen/rubato/issues/248)) | **done** (#252, 2026-08-26: the delta carries an `expansion` entry — wall-clock build time keyed by the seed's `provenance.operation` (default `muse_grow@1`), with variation-point and note counts; error paths carry no phantom timing) |
