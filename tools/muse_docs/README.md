...
| Kind | Catches |
|---|---|
| `broken-link` | a relative markdown link whose target does not exist (depth errors like `../../FORMAT_SPEC.md` from `docs/`, which escapes the repo root) |
...
`tools/muse_docs/tests/test_lint.py` — 28 tests. Each check is exercised
against a synthetic repo in `tmp_path` to prove it *catches* its drift class,
the false-positive guards are pinned, and `test_real_repo_has_no_findings`
is the regression gate on the real tree.