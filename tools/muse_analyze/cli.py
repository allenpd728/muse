...
def main(argv=None):
    ...
    ap.add_argument("--all", action="store_true",
                    help="analyze the full corpus and write docs/analysis-report.md")
...
def _run_all():
    """Analyze every corpus file and write docs/analysis-report.md."""
    report_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "docs", "analysis-report.md")
    lines = [
    ...
    for rel in CORPUS_FILES:
        work = muse_corpus.load_file(rel)
        result = analyzer.analyze(work)
        lines.append(f"## {rel}")
        lines.append("")
        lines.append(f"work: {rel}")
        lines.append(f"parts: {dict((p.id, len(p.notes)) for p in work.parts)}")
        lines.append(f"  exact repeats: {len(result.exact)} distinct patterns")
        lines.append(f"  transposed repeats: {len(result.transposed)} distinct patterns")
        lines.append(f"  ostinato (rhythm): {len(result.ostinato)} distinct patterns")
        lines.append(f"  imitative entries: {len(result.imitative)} distinct patterns")
        lines.append(f"  delta curve points: {len(result.delta_curve)}")
        lines.append("")
